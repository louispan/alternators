...
module Control.Monad.Trans.Forker where
...
newtype ForkerT m a = ForkerT
    { runForkerT ::
        -- given the ability to create a command to fork STM work.
        (forall b. STM b -> (b -> m ()) -> m ())
        -- we can create a command to call a continuation in a separate thread.
        -> (a -> m ())
        -> m ()
    }

type Forker = ForkerT Identity

-- | Thread forked STM TMVar around a continuation
forker :: Monad m => ((a -> m ()) -> m ()) -> ForkerT m a
forker m = ForkerT $ \forkStm fire ->
    forkStm newEmptyTMVar $ \v -> do
        m $ \a -> forkStm (putTMVar v a) (const $ pure ())
